...
- `DEMO_COMPOSER_PUBLIC_WEB_URL` to the browser-facing portal origin without a path, query, or hash, for example `https://demo.example.com`
- `VITE_DEMO_COMPOSER_API_URL` in the portal build to the API origin

`DEMO_COMPOSER_PUBLIC_WEB_URL` is used for browser-facing links generated by the server, such as organization invite links. If it is unset, the server falls back to the request host, which is only correct for same-origin deployments.
...
- `DEMO_COMPOSER_PUBLIC_WEB_URL`, when set, must be an absolute `http` or `https` portal origin with no path, query, or hash.
...
- set `DEMO_COMPOSER_PUBLIC_WEB_URL` to the deployed portal origin, without a path, query, or hash, when the API and portal origins differ
